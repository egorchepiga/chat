...
-- Экспортируемые данные не выделены.
-- Дамп структуры для представление test.test
-- Создание временной таблицы для обработки ошибок зависимостей представлений
CREATE TABLE `test` (
	`id` INT(11) NOT NULL,
	`login` CHAR(20) NOT NULL COLLATE 'utf8_general_ci',
	`password` CHAR(255) NOT NULL COLLATE 'utf8_general_ci',
	`firstName` CHAR(25) NOT NULL COLLATE 'utf8_general_ci',
	`lastName` CHAR(25) NOT NULL COLLATE 'utf8_general_ci',
	`date` DATETIME NULL
) ENGINE=MyISAM;
...
-- Дамп структуры для процедура test.deleteFriend
DELIMITER //
...
WHERE `user` = (select `id` from `users` where `login` = param1)
AND `friend` = (select `id` from `users` where `login` = param2);
select count(`id`)  FROM `friendList`
WHERE `user` = (select `id` from `users` where `login` = param1)
AND `friend` = (select `id` from `users` where `login` = param2) into @id;

if @id = 0 then select true as result;
...
-- Дамп структуры для процедура test.showFriends
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `showFriends`(
	IN `userName` CHAR(50)CHARSET UTF8
...
set @_user = 0;
set @_friend = 0;
select `id` from `users` where `login` = username into @id;
select `user` from `friendList` where `friend` = @id into @_user;
select `friend` from `friendList` where `user` = @id into @_friend;

select @_user as `friends` UNION select @_friend ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `test` AS select `users`.`id` AS `id`,`users`.`login` AS `login`,`users`.`password` AS `password`,`users`.`firstName` AS `firstName`,`users`.`lastName` AS `lastName`,`users`.`date` AS `date` from `users`;
