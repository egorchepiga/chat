...
BEGIN
	INSERT INTO `Image` SET `Image`.`path` = _path;
	INSERT INTO `Message` SET 
		`Message`.`idRoom` = _idRoom,
		`Message`.`idSender` = _idSender,
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `AddInRoom`(
    ...
    COMMENT 'id пользователя, id комнаты'
BEGIN
...
-- Дамп структуры для процедура CHAT.CreatRoom
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `CreatRoom`(
	IN `_idAuthor` INT,
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `GetSalt`(
    ...
    COMMENT 'id пользователя'
BEGIN
	SELECT `salt` FROM `Users` WHERE `id` = _idUser; 
END//
DELIMITER ;
